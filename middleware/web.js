...
getDesc = async (Tag) => {
    let res ='-';
    const col = dbC.collection('TAGS');
    const t = await col.findOne({ TagName: Tag });
    if(t.Description){
        res = t.Description
    }
    return res;
...
module.exports.getChartData = async (Tag, STime, ETime) => {
    STime = new Date(STime);
    ETime = new Date(ETime);
    ...
    const desc =  await getDesc(Tag);

    let rec = { Name: Tag, Description :desc, records: [] };
    const options = { sort: { TimeStamp: 1 }, projection: { _id: 0 } };
